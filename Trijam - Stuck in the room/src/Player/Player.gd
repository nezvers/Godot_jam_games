...
func _physics_process(delta):
	...
	set_animation()
	if global_position.y < highest:	#check the highest point
		highest = global_position.y
		get_parent().highest_detected = int(highest)
...
func _on_Hitbox_body_entered(body):
	Event.emit_signal("game_over", "player hit")
	remove_from_group("Player")
	set_physics_process(false) #disable process
	visible = false
